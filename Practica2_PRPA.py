...
class Monitor():
    ...
    def are_no_ped_carS(self):
        return self.npedestrian.value == 0 and self.ncarS.value == 0 #and self.ncarS_waiting.value == 0
 
    def are_no_ped_carN(self):
        return self.npedestrian.value == 0 and self.ncarN.value == 0 #and self.npedestrian_waiting.value == 0
        ...
        if (direction == 0): #Norte
            ...
            if self.ncarN.value == 0:
                if self.ncarS_waiting.value > 0:
                    self.ok_carS.notify_all() #Revisar el all, pero me interesa que salgan todos 
                else:
            ...
            if self.ncarS.value == 0:
                if self.npedestrian_waiting.value > 0:
                    self.ok_pedestrian.notify_all() #Revisar el all, pero me interesa que salgan todos 
                else:
                    self.ok_carN.notify_all()
        self.mutex.release()
    
    def are_no_cars(self):
        return self.ncarN.value == 0 and self.ncarS.value == 0 #and self.ncarN_waiting.value == 0
    
    def wants_enter_pedestrian(self) -> None:
        self.mutex.acquire()
        self.npedestrian_waiting.value += 1
        self.ok_pedestrian.wait_for(self.are_no_cars) #Da preferencia si hay coches por arriba esperando
        self.npedestrian_waiting.value -= 1
    ...
    def leaves_pedestrian(self) -> None:
        ...
        if self.npedestrian.value == 0:
            if self.ncarN_waiting.value > 0:
                self.ok_carN.notify_all() #Revisar el all, pero me interesa que salgan todos 
            else:
